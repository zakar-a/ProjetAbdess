...
const MonthlyInvoiceTemplate = ({ clientId, clientName, month, year, monthOrders }) => {
  const { products } = useAppContext();

  if (!monthOrders || monthOrders.length === 0) return null;
  ...
  monthOrders.forEach(order => {
    order.items.forEach(item => {
      if (!aggregated[item.productId]) {
        aggregated[item.productId] = { productId: item.productId, quantity: 0, totalLine: 0 };
      }
      const prod = products.find(p => p.id === item.productId);
      const unitPrice = prod?.price || 0;
      aggregated[item.productId].quantity += item.quantity;
      aggregated[item.productId].totalLine += item.quantity * unitPrice;
    });
  });

  const aggregatedItems = Object.values(aggregated);
  const grandTotal = monthOrders.reduce((sum, o) => sum + o.total, 0);
  const periodLabel = `${MONTH_NAMES[month]} ${year}`;
  const invoiceRef = `FM-${String(month + 1).padStart(2, '0')}${year}-${clientId?.toUpperCase().slice(0, 4)}`;

  return (
    <div className="invoice-container" style={{ fontFamily: 'Inter, sans-serif', color: '#1a1a1a', padding: '40px' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', borderBottom: '3px solid #E2725B', paddingBottom: '20px' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <h2 style={{ fontSize: '1.2rem', color: '#E2725B', margin: '0 0 5px 0' }}>FACTURE MENSUELLE</h2>
          <p style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700 }}>Réf: {invoiceRef}</p>
          <p style={{ margin: 0, fontSize: '0.9rem', color: '#7f8c8d' }}>Période: {periodLabel}</p>
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#7f8c8d' }}>Édité le: {new Date().toLocaleDateString()}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px', background: '#FDF7F5', padding: '20px', borderRadius: '12px' }}>
        ...
        <div style={{ width: '45%', textAlign: 'right' }}>
          ...
          <p style={{ fontWeight: 800, fontSize: '1.1rem', margin: '0 0 5px 0' }}>{clientName}</p>
          ...
          <p style={{ fontSize: '0.9rem', margin: 0, color: '#444' }}>ID Client: {clientId}</p>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '30px', padding: '15px', background: '#f0f0f0', borderRadius: '8px' }}>
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#555' }}>
          <strong>{monthOrders.length}</strong> commande(s) sur la période <strong>{periodLabel}</strong>
        </p>
      </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
        <thead>
          <tr style={{ background: '#2C3E50', color: 'white' }}>
            <th style={{ textAlign: 'left', padding: '15px', borderRadius: '8px 0 0 0' }}>Description du Produit</th>
            <th style={{ textAlign: 'center', padding: '15px' }}>Unité</th>
            <th style={{ textAlign: 'center', padding: '15px' }}>Quantité Totale</th>
            ...
            return (
              <tr key={idx} style={{ background: idx % 2 === 0 ? 'white' : '#F9F9F9' }}>
                <td style={{ padding: '15px', borderBottom: '1px solid #EEE' }}>
                  <p style={{ fontWeight: 700, margin: 0 }}>{prod?.name || item.productId}</p>
                  <p style={{ fontSize: '0.75rem', color: '#7f8c8d', margin: 0 }}>Cumul mensuel - Qualité A</p>
                </td>
                ...
                <td style={{ textAlign: 'right', padding: '15px', borderBottom: '1px solid #EEE', fontWeight: 800 }}>{item.totalLine}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <div style={{ marginBottom: '40px' }}>
        <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E50', textTransform: 'uppercase', marginBottom: '10px' }}>Détail par commande</p>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
          <thead>
            <tr style={{ background: '#ecf0f1' }}>
              <th style={{ textAlign: 'left', padding: '10px' }}>Réf</th>
              <th style={{ textAlign: 'left', padding: '10px' }}>Date</th>
              <th style={{ textAlign: 'left', padding: '10px' }}>Magasin</th>
              <th style={{ textAlign: 'right', padding: '10px' }}>Montant</th>
            </tr>
          </thead>
          <tbody>
            {monthOrders.map((o, idx) => (
              <tr key={o.id} style={{ background: idx % 2 === 0 ? 'white' : '#F9F9F9' }}>
                <td style={{ padding: '8px 10px', borderBottom: '1px solid #EEE' }}>#{o.id}</td>
                <td style={{ padding: '8px 10px', borderBottom: '1px solid #EEE' }}>{new Date(o.timestamp).toLocaleDateString()}</td>
                <td style={{ padding: '8px 10px', borderBottom: '1px solid #EEE' }}>{o.magasinName}</td>
                <td style={{ padding: '8px 10px', borderBottom: '1px solid #EEE', textAlign: 'right', fontWeight: 600 }}>{o.total} DH</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '60px' }}>
        <div style={{ width: '300px', background: '#FDF7F5', padding: '20px', borderRadius: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: '1rem' }}>
            ...
            <span style={{ fontWeight: 700 }}>{grandTotal} DH</span>
          </div>
      ...
      {/* Terms & Signature — Abdelssalam */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div style={{ fontSize: '0.8rem', color: '#7f8c8d', maxWidth: '350px', fontStyle: 'italic' }}>
          <p style={{ fontWeight: 700, color: '#E2725B', marginBottom: '5px' }}>CONDITIONS :</p>
          <p>Facture mensuelle regroupant l'intégralité des commandes passées sur la période indiquée. Paiement à 30 jours à compter de la date d'édition.</p>
          <p style={{ marginTop: '15px' }}>Merci de votre confiance.</p>
        </div>
